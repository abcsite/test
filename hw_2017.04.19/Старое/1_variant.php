...
function getCommonWords($a, $b) {
    ...
    foreach ($arr_a as $vol_a) {
        $flag = false;
        foreach ($arr_b as $vol_b) {
            if ($vol_a === $vol_b) {
                $flag = true;
            }
        };
        // Проверяет, есть ли это слово в массиве с общими словами.
        // Это исключает повторения одинаковых слов в массиве с ообщими словами.
        if ($flag) {
            foreach ($arr_common as $vol_com) {
                if ($vol_a === $vol_com) {
                    $flag = false;
                }
            };
        }
        if ($flag) { $arr_common[] = $vol_a; }
    };
    return $arr_common;
}
